...
import eggman89.genreReco._
import eggman89.{genreReco, hashmap}
...
object tagGenerator {
  def main(args: Array[String]){
    ...
    val map_tagid_tag0 = new genreReco.hashmap()
    val map_tagid_tag = new genreReco.hashmap()
    ...
    val tid_trackid = sc.textFile("C:/Users/sowmy/Google Drive/Dataset/track_id_to_tag.txt").map(_.split("\t")).map(p => Row(p(0), map_tagid_tag0.add(p(1))))
    val df_track_id_tag_id = sqlContext.createDataFrame(tid_trackid, schema)
    val temp = sc.textFile("C:/Users/sowmy/Google Drive/Dataset/track_id_to_tag.txt").collect().map(_.split("\t")).map(p => Row(p(0), map_tagid_tag.add(p(1))))
    ...
    val df_attributes =  sqlContext.load("com.databricks.spark.csv", Map("path" -> "C:/Users/sowmy/Google Drive/Dataset/song_attributes.csv", "header" -> "true"))
